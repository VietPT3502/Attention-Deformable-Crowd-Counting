...
def process(idx, img_paths):
    ...
    with h5py.File(img_path.replace('.jpg','.h5').replace('images','Annotations_h5'), 'w') as hf:
        hf['density'] = k

    print(idx,len(img_paths))

root = "DroneCrowd"
# part_train = os.path.join(root, 'train','images')
part_val = os.path.join(root, 'val','images')
# part_test = os.path.join(root, 'test','images')
...
for img_path in glob.glob(os.path.join(part_val, '*.jpg')):
    img_paths.append(img_path)
img_paths.sort()
